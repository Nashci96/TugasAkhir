...
//        tvLoginMhs  = findViewById(R.id.tvLoginMhs);
                        ...
                        try {
                            ...
                            if (success.equals("1")) {
                                for (int i = 0; i < jsonArray.length(); i++) {
                                    ...
                                    FirebaseInstanceId.getInstance().getInstanceId().addOnSuccessListener(LoginActivityMhs.this,
    ...
    private void updateToken(final String id, final String newToken) {
        StringRequest stringRequest = new StringRequest(Request.Method.POST, URL.MhsToken,
                new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        try {
                            ...
                            if (success.equals("1")) {
                                Toast.makeText(LoginActivityMhs.this, "Success!", Toast.LENGTH_LONG).show();
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                            Toast.makeText(LoginActivityMhs.this, "Error : " + e.toString(), Toast.LENGTH_LONG).show();
                        }
                    }
                },
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(LoginActivityMhs.this, "Error : " + error.toString(), Toast.LENGTH_LONG).show();
                    }
                })
        {
            @Override
